...
exports.unlistBrand = catchAsync(async (req, res, next) => {
    const { id } = req.params;
    const brand = await Brand.findByIdAndUpdate(id, { listed: false }, { new: true });
    if (!brand) return next(new AppError('Brand not found', 404));
    res.json({ message: 'Brand unlisted', brand });
});
